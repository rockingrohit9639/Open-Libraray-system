...
const register = async (req, res) => {
  try {
    ...
    if (!full_name) {
      return res
        .status(404)
        .json({ status: 404, message: "Full name required" });
    }
    if (!email) {
    ...
    if (!password) {
      return res
        .status(404)
        .json({ status: 404, message: "password required" });
    }
    ...
    if (emailMatched) {
      return res
        .status(400)
        .json({ status: 400, message: "Email id is already registered" });
    }
    if (phoneMatched) {
      return res
        .status(400)
        .json({ status: 400, message: "Phone number already registered" });
    }
    ...
    return res
      .status(500)
      .json({ status: 500, message: "Server internal problem" });
  }
};
...
const login = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ status: 400, message: "Email doesn't exist in our database" });
    }
    ...
    if (passwordMatched) {
      ...
      return res
        .status(404)
        .json({ status: 404, message: "Email or password is not matching" });
    }
  } catch (error) {
    return res
      .status(500)
...
const forget = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: 200, message: "Check mail ", token: token });
  } catch (error) {
    return res
      .status(500)
      .json({ status: 500, message: "Server internal problem" });
...
const resendForget = async (req, res) => {
  try {
    ...
    if (!email) {
      return res
        .status(404)
        .json({ status: 404, message: "Email is required" });
    }
    const user = await User.findOne({ email });
    if (!user) {
...
const changePassword = async (req, res) => {
  try {
    ...
    if (!password) {
      return res
        .status(404)
        .json({ status: 404, message: "Password is required" });
    }
    if (!new_password) {
      return res
        .status(404)
        .json({ status: 404, message: "New password is required" });
    }
    if (new_password.length < 8) {
      return res
        ...
        .json({
          status: 400,
          message: " The password must contain at least three character ",
        });
    }
    const user = await User.findOne({ _id: req.user._id });
    if (await bcrypt.compare(password, user.password)) {
      ...
      return res
        .status(200)
        .json({ status: 200, message: "Password has been updated" });
    } else {
...
const resetPassword = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: 200, message: "Password has been changed" });
  } catch (error) {
    return res
      .status(500)
      .json({ status: 500, message: "Server internal problem" });
